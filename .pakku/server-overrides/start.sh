...
installJava() {
  ...
  . install_java.sh || crashServer "Java install-script failed. Install Java $RECOMMENDED_JAVA_VERSION manually or edit JAVA in your _variables.txt to point to a Java installation of said version."
...
# Depending on the value of SERVERSTARTERJAR_FORCE_FETCH in the _variables.txt the server.jar is force-refreshed.
# Meaning: If true, the server.jar will be deleted and then downloaded again.
# Depending on the value of SERVERSTARTERJAR_VERSION in the _variables.txt a different version is fetched. More on
# this value in the _variables.txt
refreshServerJar() {
...
setupNeoForge() {
  ...
  echo "Edit JAVA_ARGS in your _variables.txt. Do not edit user_jvm_args.txt directly!"
...
echo "Start script generated by ServerPackCreator 8.1.0."
echo "To change the launch settings of this server, such as JVM args / flags, Minecraft version, modloader version etc., edit the _variables.txt-file."
...
if [[ ! -s "_variables.txt" ]]; then
  crashServer "ERROR! _variables.txt not present. Without it the server can not be installed, configured or started."
fi
